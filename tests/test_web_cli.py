...
from ptliq.cli.web import app as web_app

runner = CliRunner(mix_stderr=False)


def test_web_stop_no_pidfile(tmp_path: Path):
    # Point the CLI to a temp pidfile that does not exist
    env = os.environ.copy()
    pidfile = tmp_path / "pidfile.pid"
    env["PTLIQ_WEB_PIDFILE"] = str(pidfile)

    res = runner.invoke(web_app, ["stop"], env=env)
    # Should be a clean no-op
    assert res.exit_code == 0, res.stdout
    assert not pidfile.exists()


def test_web_stop_kills_process(tmp_path: Path):
    ...
    try:
        ...
        res = runner.invoke(web_app, ["stop", "--timeout", "2"], env=env)
        assert res.exit_code == 0, res.stdout
