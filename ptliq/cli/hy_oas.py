...
def fetch(
    ...
    """
    Fetch ICE BofA US High Yield Option-Adjusted Spread (FRED: BAMLH0A0HYM2).
    """
    start_ts, end_ts = _infer_date_range(years, start, end)
    print(f"[bold]Fetching HY OAS (BAMLH0A0HYM2)[/bold] from [cyan]{start_ts.date()}[/cyan] to [cyan]{end_ts.date()}[/cyan]")
    try:
        df = fetch_fred_series("BAMLH0A0HYM2", start_ts, end_ts)
